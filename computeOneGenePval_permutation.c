...
void doLinearModel ( double *myarr, int numInd, int numCov, double *y , double *beta ,double *snp_stderr, double *pvalue) {
	...
	*beta = betaAsIs; 
	...
	/*const int num = 1; 
	double snp_ps = 0; 
	double temp = -1.0*fabs(stdBeta); 
	vdCdfNorm(num,&temp,&snp_ps); // lower tail times 2 
	snp_ps = snp_ps*2.0; 
	*/
	*pvalue = snp_ps;
...
void print_help(void) {
  fprintf(stderr,"Usage: ols_perm [options]\n");
  fprintf(stderr,"  --expr [expression file]\n");
  fprintf(stderr,"  --geno [genotype file]\n");
  fprintf(stderr,"  --output [out prefix]\n");
  fprintf(stderr,"  --numPerm [# of permutations]\n");
  fprintf(stderr,"  --numSNP [# of SNPs]\n");
...
int main(int argc, char** argv) {
	...
	/*//FILE* nn = fopen ("testthetcdf2.txt", "w"); 
	//double *p ; 
	double test = 0 ; 
	//p=test;
	int mm = 0;
	for (mm=0; mm<100; mm++){
		test = test + .1; 
		//double prob = tcdf(test, 97-22); 
		double prob = 0; 
		//double * p2 ; 
		//p2 = prob; 
		const int num = 1;
		vdCdfNorm(num,&test,&prob); //vdCdfNorm(numSNPInWindow[j], zscoreWindow[j], pvalueWindow[j]);
		fprintf(nn,"%f\t%f\n", test, prob);
	}
	//fclose(nn); */
    ...
    {"gradient", required_argument, 0, 'd' },
	...
	{"optimize", required_argument,0,'n'},
	{"permutate", required_argument,0,'o'},
	...
	// before reading weights, need to know the numSNP, one dumb ass way is to read the weight file and count the lines 
	/*FILE* fcountSnp = fopen(wtsf,"r"); 
	char snpCountLine [SZ_LONG_BUF];
	numSNP = 0 ; 
	while (fgets(snpCountLine, SZ_LONG_BUF, fcountSnp) != NULL) {
		numSNP = numSNP + 1; 
	} 
	fclose(fcountSnp);
	*/

	// read in the weights here....
	printf ("Read priors.\n"); 
	float *snp_weights ; 
...
/*
	float *snp_weights = (float*)&array ;
	float sum_w = (float)(1.0 * numSNP); 
	for (w=0; w<numSNP; w++) {
		//sum_w += *(snp_priors+w); 
    sum_w += 1.0; 
	}*/
	...
	//if (sum_w != 1) {
	/*	for (w=0; w<numSNP; w++) {
			//float temp = *(snp_priors+w)/sum_w;
      		float temp = (float)(1.0 /sum_w);
			*(snp_weights+w) =  temp; // all priors are equal. 
			//printf ("w %f\n",*(snp_weights+w)) ; 
		}
	//}
*/
	// begin compute gradient
	...
	for ( i = 0 ; i < numSNP; i++){ // over each SNP 
		...
		//printf ("std beta %f\n", sdzBeta);
		//pvalue = tcdf( fabs(sdzBeta) , (numInd-numCov-2));

		//gradient = computeGradient ( sdzBeta, MEAN, *(snp_weights+i)) ; // STANDARDIZED BETA, SD=1

		// original pvalue is wrt t-density 
		// normal-density. is 1-to-1, so take this pvalue as an input to the normal-density
        ...
        if (optimize==1){
            ...
            gradient = computeGradient ( this_beta, this_beta, *(snp_weights+i)) ; // STANDARDIZED BETA, SD=1
        }
        else if (optimize==0){
            gradient = computeGradient ( this_beta, MEAN, *(snp_weights+i)) ; // STANDARDIZED BETA, SD=1
        }
        else {
            printf("\noptimize indication not recognized.\n");
        }
        
        if (gradient > maxLlh){ // most significant
			maxLlh = gradient;  
		}
		if (pvalue < minPval ) {
			minPval = pvalue; 
			...
			min_snp_pos = snpps; 
			//best_beta = sdzBeta; 
			//beststderr = snp_stderr;
		}
		if (fabs(sdzBeta) > maxBeta ) {
			maxBeta = fabs(sdzBeta); 
		}
		distance = geneps - snpps ; // tss - snp 
		fprintf (ob, "%d\t%d\t%f\t%f\t%f\t%f\n", snpps, distance, beta,snp_stderr, this_beta, gradient);
	}
	//printf("\n beta: %f, stderr: %f\n", best_beta, beststderr); 
	//printf ("snp index with min p-value: %d, pos %d, min p-value: %f\n\tbeta: %f, stderr: %f\n", min_index, min_snp_pos, minPval,best_beta, beststderr); 
	//printf ("\n%f\n", maxBeta); 
	...
	/*FILE *fout3 = fopen(obsValues_outFile,"w+");
	fprintf(fout3, "%f\n", maxBeta);
	fclose(fout3);
	*/
	...
	*/
	...
	if (numPermu > 0){
		...
		//double pbefore = 1; 
		//double pafter = 1; // see if change in ratio is small, then quit. 
		...
		for ( numP = 0; numP < numPermu; numP++) {
			...
			for ( i = 0 ; i < numSNP; i++){ // over each SNP 
				...
				if (gradient > bestGradSim){ // most significant  
					bestGradSim = gradient;  
				}
				//if (fabs(sdzBeta) > bestBetaSim ) {
				//	bestBetaSim = fabs(sdzBeta); 
				//}
				if (pvalue < bestPvalSim) {
					bestPvalSim = pvalue; 
				}
			}
			// compare to best observed beta 
			//if (bestBetaSim > maxBeta) {
			//	compareCounter_beta = compareCounter_beta + 1; 
			//}
			...
			//fprintf(fout3, "%f\t%f\n", bestPvalSim,bestGradSim); // print the min-p and max llh at each round.

			/*if (numP>500000){
				pafter = (double)compareCounter/numP;
				double ratio = (pafter - pbefore)/pbefore; 
				double r = fabs(ratio); 
				if (r < 0.001) {
					numPermu = numP+1 ; // if stop at numP=100, that means we did 101 iterations  
					break; 
				}
			}
			pbefore = (double)compareCounter/numP;
			*/
		}	
		//fclose(fout3);
		...
		//double p3 = (double)compareCounter_beta/numPermu;  
		printf ("min-p pvalue%f\n", p1 ); 
		printf("llh pvalue %f\n", p2 );
		//printf("z-score pvalue %f\n", p3 );
...
float * read_wts ( char* file, float* arr, int stop ){
	...
	if((fp=fopen(file,"r"))==NULL){
			printf("Cannot open priors input...?\n");
			exit(1);
	}
	int i = 0; 
	for (i=0; i<stop; i++){
